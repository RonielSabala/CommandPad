...
interface KeybindingDef {
  binding: string;
  description: string;
  /** false → display only (not wired to a keyboard listener). */
  keyboard?: boolean;
}

export const KEYBINDINGS: Record<KeyBinding, KeybindingDef> = {
  [KeyBinding.TOGGLE_MODE]: { binding: 'ctrl+e', description: 'Toggle read / edit mode' },
  [KeyBinding.NEW_TAB]: { binding: 'ctrl+alt+n', description: 'Open a new tab' },
  [KeyBinding.CLOSE_TAB]: { binding: 'ctrl+alt+w', description: 'Close the active tab' },
  [KeyBinding.TOGGLE_EDITORS]: { binding: 'ctrl+g', description: 'Toggle all command editors' },
  [KeyBinding.DELETE_RUNBOOK]: {
    binding: 'ctrl+shift+d',
    description: 'Delete the focused runbook from the library',
  },
  [KeyBinding.IMPORT_RUNBOOK]: { binding: 'ctrl+i', description: 'Open runbook import dialog' },
  [KeyBinding.TOGGLE_SIDEBAR]: { binding: 'ctrl+s', description: 'Collapse / expand sidebar' },
  [KeyBinding.MOVE_SIDEBAR]: { binding: 'ctrl+shift+s', description: 'Move sidebar to left / right' },
  [KeyBinding.DUPLICATE_BLOCK]: { binding: 'ctrl+d', description: 'Duplicate selected blocks' },
  [KeyBinding.DELETE_BLOCK]: { binding: 'delete', description: 'Delete selected blocks' },
  [KeyBinding.ESCAPE]: { binding: 'escape', description: 'Clear block selection / close modals' },
  [KeyBinding.EXPORT]: { binding: 'ctrl+shift+e', description: 'Open export dialog' },
  [KeyBinding.CLEAR_WORKSPACE]: {
    binding: 'ctrl+shift+backspace',
    description: 'Open clear workspace dialog',
  },
  [KeyBinding.FOCUS_RUNBOOK]: { binding: 'tab', description: 'Select active runbook' },
...
/** Pretty-print a binding string for display in the keybindings modal. */
export function formatBinding(binding: string): string {
  ...
  return binding
    .split(' ')
    .map((part) => (part.includes(BINDING_SEPARATOR) || /[a-z]/.test(part) ? titleCaseChord(part) : part))
    .join(' ');
}
...
const parsedBindings: Partial<Record<KeyBinding, ParsedBinding>> = Object.fromEntries(
  Object.entries(KEYBINDINGS)
    ...
    .map(([id, { binding }]) => {
      ...
      const modifiers = Object.fromEntries(MODIFIER_KEYS.map((mod) => [mod, parts.includes(mod)]));
      return [id, { ...modifiers, key: KEY_ALIASES[rawKey] ?? rawKey }];
    }),
);

/** Does a keyboard event match the given binding id? */
export function matchesKeybinding(event: KeyboardEvent, bindingId: KeyBinding): boolean {
